...
public class adminUser extends user {
    ...
    @Override
    public void deleteReg(HttpSession session,String Rnum,String type) {
        init();
        String table=null;
        ...
        if(type.equalsIgnoreCase("student"))
            ...
            table="teacher_info";
        try {
            rs = st.executeQuery("SELECT * FROM `"+table+"` WHERE registration_number like '"+Rnum+"'");
            if(rs.next())
            {
            st.execute("Delete FROM `"+table+"` WHERE registration_number like '"+Rnum+"'");
            st.execute("DELETE FROM `login` WHERE reg_num like '"+Rnum+"'");
            session.setAttribute("del_msg", "Successfully deleted");
            }
            else{
            session.setAttribute("del_msg", "Registration number not found!");
            }
            
        } catch (SQLException ex) {
           session.setAttribute("del_msg", "Error occur!!!"+ex);
        }
    }
    ...
    public void profile(HttpSession session) {
        ...
        try {
            ResultSet rs=st.executeQuery(" SELECT * FROM `admin_info` WHERE registration_number like '"+reg_num+"'");
            if(rs.next()){
